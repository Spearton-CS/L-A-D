...
public class Missile : Enemies.Enemy
{
    ...
    public static GameObject CreateArrow(Vector3 pos, Quaternion rotation = new()) => Instantiate(SceneManager.GetActiveScene().GetRootGameObjects().Where((x) => x.name == "Arrow").First(), pos, rotation);
}